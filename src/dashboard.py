...
if(btnAtu)btnAtu.onclick=async()=>{btnAtu.disabled=true;elStatus.className='';elStatus.textContent='Consultando a Embrapa ClimAPI…';
  try{const r=await fetch('api/atualizar',{method:'POST'});const novo=await r.json();
  ...
  catch(e){elStatus.className='err';elStatus.textContent='Não foi possível atualizar ao vivo ('+e.message+'). Precisa do servidor: python -m src.servidor';}
...
function faixaDia(pts,c){const n=pts.length,W=1000,H=44,pl=4,pr=4,hb=26,iw=(W-pl-pr)/Math.max(1,n);let rects='',ticks='';
  pts.forEach((p,i)=>{const x=pl+i*iw;rects+='<rect x="'+(x+.5)+'" y="0" width="'+Math.max(.5,iw-1)+'" height="'+hb+'" rx="1.5" fill="'+STATUS[p.classe]+'" data-i="'+i+'"></rect>';
    const h=new Date(p.data).getHours();if(h%3===0)ticks+='<text x="'+(x+iw/2)+'" y="'+(H-4)+'" text-anchor="middle" fill="var(--muted)" font-size="10">'+dois(h)+'h</text>';});
...
function renderTimelines(){const host=$('timelines');if(!host)return;host.innerHTML='';
  ativasLista().forEach(c=>{const serie=c.serie,n=serie.length,W=1000,H=34,pl=4,pr=4,hb=24,iw=(W-pl-pr)/Math.max(1,n);let rects='',ticks='',ult='';
    serie.forEach((p,i)=>{const x=pl+i*iw;rects+='<rect x="'+(x+.5)+'" y="0" width="'+Math.max(.5,iw-1)+'" height="'+hb+'" rx="1.5" fill="'+STATUS[p.classe]+'" data-i="'+i+'"></rect>';});
    serie.forEach((p,i)=>{const d=new Date(p.data);if(d.toDateString()!==ult&&d.getHours()<3){const x=pl+i*iw;
      ticks+='<line x1="'+x+'" y1="0" x2="'+x+'" y2="'+hb+'" stroke="var(--surface)" stroke-width="2"/><text x="'+(x+3)+'" y="'+(H-2)+'" fill="var(--muted)" font-size="11">'+d.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})+'</text>';ult=d.toDateString();}});
...
function chartSVG(m,cidades){const W=500,H=250,pl=40,pr=12,pt=12,pb=26,iw=W-pl-pr,ih=H-pt-pb;
  ...
  s+='<rect x="'+pl+'" y="'+yTop+'" width="'+iw+'" height="'+Math.max(0,yBot-yTop)+'" fill="var(--band)"/>';
  ...
  for(let g=0;g<=4;g++){const v=ymin+(ymax-ymin)*g/4,y=Y(v);
    s+='<line x1="'+pl+'" y1="'+y+'" x2="'+(W-pr)+'" y2="'+y+'" stroke="var(--grid)" stroke-width="1"/><text x="'+(pl-6)+'" y="'+(y+4)+'" text-anchor="end" fill="var(--muted)" font-size="11">'+v.toFixed(m.dec)+'</text>';}
  let ult='';ref.forEach((p,i)=>{const d=new Date(p.data);if(d.toDateString()!==ult&&d.getHours()<3){s+='<text x="'+X(i)+'" y="'+(H-8)+'" fill="var(--muted)" font-size="11">'+d.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})+'</text>';ult=d.toDateString();}});
  cidades.forEach((c,ci)=>{const cor=CORES[idxCor(c)];let d='';sc[ci].forEach((p,i)=>{d+=(i?'L':'M')+X(i).toFixed(1)+' '+Y(p[m.key]).toFixed(1)+' ';});
  ...
  s+='<line id="cx" x1="0" y1="'+pt+'" x2="0" y2="'+(pt+ih)+'" stroke="var(--axis)" stroke-width="1" opacity="0"/><rect x="'+pl+'" y="'+pt+'" width="'+iw+'" height="'+ih+'" fill="transparent"/></svg>';
...
const lref=(x,w,y)=>'<line x1="'+x+'" y1="'+y+'" x2="'+(x+w)+'" y2="'+y+'" stroke="var(--azul)" stroke-width="1" stroke-dasharray="4 3" opacity="0.45"/>';
...
function renderBarrasDia(){const host=$('barras-dia');if(!host)return;const cid=DADOS.cidades;const dias=diasDisponiveis();
  ...
  let s='<svg viewBox="0 0 '+W+' '+H+'" role="img">';
  for(let g=0;g<=4;g++){const v=maxV*g/4,y=pt+ih-(v/maxV)*ih;s+='<line x1="'+pl+'" y1="'+y+'" x2="'+(W-pr)+'" y2="'+y+'" stroke="var(--grid)"/><text x="'+(pl-6)+'" y="'+(y+4)+'" text-anchor="end" fill="var(--muted)" font-size="11">'+Math.round(v)+'</text>';}
  dias.forEach((d,gi)=>{const x0=pl+gi*gw+(gw-bw*cid.length)/2;
    cid.forEach((c,ci)=>{const v=dados[gi][ci],h=(v/maxV)*ih,x=x0+ci*bw,y=pt+ih-h;
      s+='<rect x="'+(x+1)+'" y="'+y+'" width="'+(bw-2)+'" height="'+h+'" rx="2" fill="'+CORES[idxCor(c)]+'"><title>'+c.nome+' • '+v+' favoráveis</title></rect>';});
    const dd=new Date(d+'T00:00');s+='<text x="'+(pl+gi*gw+gw/2)+'" y="'+(H-12)+'" text-anchor="middle" fill="var(--muted)" font-size="11">'+dd.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})+'</text>';});
...
$('tema').onclick=()=>{const cur=document.documentElement.getAttribute('data-theme');document.documentElement.setAttribute('data-theme',cur==='dark'?'light':'dark');render();};
